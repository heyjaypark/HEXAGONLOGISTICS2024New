...
public class ProductLogService {
	...
	public ProductLogPage getProductPage(int pageNum) {
		try (Connection conn = ConnectionProvider.getConnection()) {
			// 전체 상품 수 조회
			// 全体の商品数を取得
			int total = productlogDao.selectCount(conn);
			System.out.println("productListService:" + pageNum);
			// 현재 페이지에 해당하는 상품 목록 조회
			// 現在のページに対応する商品リストを取得

			List<ProductLogRequest> content = productlogDao.select1(conn, total-(pageNum)*(size), total-(pageNum-1)*(size)-1);
